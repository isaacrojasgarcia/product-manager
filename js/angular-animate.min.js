...
(function(F, e, O) {
	...
	e.module("ngAnimate", ["ng"]).directive("ngAnimateChildren", function() {
		...
		function(G, s) {
			...
			G.decorator("$animate", ["$delegate", "$injector", "$sniffer", "$rootElement", "$$asyncCallback", "$rootScope", "$document",
				function(z, F, $, R, E, H, O) {
					...
					function G(a, b, c) {
						function d(a, b) {
							...
							if (c || d) return "leave" == b && (d = c, c = null), n.push({
								event: b,
								fn: c
							}), h.push({
								event: b,
			...
			s.register("", ["$window", "$sniffer", "$timeout", "$$animateReflow",
				function(l, n, s, B) {
					function E(a, U) {
						S && S();
						W.push(U);
